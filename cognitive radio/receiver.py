...
class OFDM_Decoder:
    ...
    def __init__(self, demodFunc, f_list, symbol_length, ss_pulse, fs = 44100.0):
       #self.f0 = f0
       #self.f1 = f1
        self.fs = fs
        self.f_list = f_list
        self.channels = f_list.size
        self.symbol_length = symbol_length
        self.symbol_size = int(fs*symbol_length)
       # self.sync_pulse = sync_pulse
        self.sync_width = 100
        ...
        # State 1
            ...
            if num_symbols > 0:
                ...
                if demod != None:
                    ...
                    if num_demod == num_symbols * self.channels:
                        self.rec_data = np.append(self.rec_data, demod)
                        self.start_idx += self.symbol_size*num_symbols
                        self.decode_count += num_symbols
    ...
    def checkForSync(self):
        if self.process_state == 0: # look for the start pulse
            ...
            if toEnd != -1:
                ...
                self.process_state = 1
                self.dataBuffer = self.dataBuffer[self.sync_width*self.symbol_size:]
                self.start_idx = 0
                return True
    ...
    def demodulate(self,data):
        return self.demodFunc(data, self.f_list, self.fs, self.symbol_length, self.delay)
